...
class VoiceActivityDetector():
    """ Use signal energy to detect voice activity in wav data (mono) """
...
#        return self
    ...
    def detect_speech(self):
        ...
        data = self.data   # with labels
        sample_start = 0
        start_band = self.speech_start_band
        end_band = self.speech_end_band
        data_label = np.empty((0,1))
        while (sample_start < (len(data) - sample_window)):
            if sample_start > 0 and sample_start % (100*sample_window) == 0:
                print(sample_start / len(data) * 10, '% finished')
            sample_end = sample_start + sample_window
            if sample_end>=len(data): sample_end = len(data)-1
            data_window = data[sample_start:sample_end, 0]
            data_label = np.append(data_label, int(round(np.sum(data[sample_start:sample_end, 1])/(sample_end - sample_start))))
        ...
        data_label = np.reshape(data_label, (len(data_label), 1))
        print(detected_windows.shape, data_label.shape)
        detected_windows = np.hstack((detected_windows, data_label))
